...
function sheet(){
 ...
 document.body.classList.toggle('journey-active',!!journey);$('journeyStatus').hidden=!journey;$('journeyLog').hidden=!journey;
 ...
 const j=journey;const elapsed=`${Math.floor(j.elapsedMonths/12)}y ${j.elapsedMonths%12}m`;
 ...
 $('journeySheet').innerHTML=`<section class="sheet-group"><h3>LIFE AT SEA</h3><dl>${row('Combat rating',combatPower(j))}${row('Berries',money(j.berries))}${row('World danger',`${j.danger}/5`)}${row('Wins / losses',`${j.wins} / ${j.losses}`)}${row('Opponents killed',j.kills)}${row('Group',j.group||'Traveling alone')}${row('Group support',j.groupSupport?'Existing group support +3':'Named companions only')}</dl></section><section class="sheet-group journey-skills"><h3>PRACTICE & GROWTH</h3>${Object.keys(j.skills).map(k=>{const rank=levelOf(j,k),track=trackFor(k),max=rank===track.length-1,low=XP_LEVELS[rank],high=XP_LEVELS[rank+1]||low;return `<div class="skill-track"><div><span>${esc(skillLabel(k))}</span><small>${esc(track[rank])}</small></div><progress max="${max?1:high-low}" value="${max?1:j.skills[k]-low}" aria-label="${esc(skillLabel(k))} progress"></progress><small>${max?'Maximum mastery':`${j.skills[k]-low} / ${high-low} practice to ${esc(track[rank+1])}`}</small></div>`;}).join('')}</section><section class="sheet-group"><h3>INVENTORY</h3>${j.inventory.length?j.inventory.map(i=>`<div class="crew-person">${esc(i.name)}${i.count>1?` × ${i.count}`:''}<small>${esc(i.type)}</small></div>`).join(''):'<p class="empty-state">Nothing carried yet.</p>'}</section><section class="sheet-group"><h3>COMPANIONS · ${j.crew.length}</h3>${j.crew.length?j.crew.map(c=>`<div class="crew-person">${esc(c.name)}<small>${esc(c.role)}${c.fruit?` · ${esc(c.fruit)}`:''}</small></div>`).join(''):`<p class="empty-state">${j.groupSupport?'Your existing group provides support. No named recruits yet.':'You travel without companions.'}</p>`}</section>`;
 renderTimeline();
}
function renderTimeline(){
 if(!journey)return;const j=journey;$('journeyLogCount').textContent=`${j.chapter} completed ${j.chapter===1?'chapter':'chapters'}`;
 $('timeline').innerHTML=j.log.length?[...j.log].reverse().slice(0,visibleChapters).map(e=>`<article class="timeline-entry ${e.alive?'':'death'}"><div class="timeline-stamp">CHAPTER ${String(e.chapter).padStart(2,'0')}<b>Age ${ageLabel(e.age)}</b><span>+${e.months} months</span></div><div class="timeline-body"><h3>${esc(e.title)}</h3><p>${esc(e.result)}</p>${e.effects.length?`<ul>${e.effects.map(t=>`<li>${esc(t)}</li>`).join('')}</ul>`:''}<details><summary>See all ${e.rolls.length} fate rolls</summary>${e.rolls.map(r=>`<div class="roll-trace"><span>${esc(r.wheel)}<br><b>${esc(r.label)}</b>${r.modifier?`<small>${esc(r.modifier.label)}: ${esc(r.modifier.outcome)} ${pct(r.modifier.before)} → ${pct(r.modifier.after)}</small>`:''}</span><span>${pct(r.chance)}</span></div>`).join('')}</details></div></article>`).join(''):'<p class="empty-state">The horizon is open. Your first event begins your journey.</p>';
 $('moreHistory').hidden=j.log.length<=visibleChapters;
}
function context(s){
 $('chapterContext').hidden=!journey||!journey.pending;
 if(journey?.pending){const p=journey.pending;$('chapterContext').innerHTML=`<b>CHAPTER ${journey.chapter+1} · ${esc(EVENT_BY_ID[p.event]?.label||'Captivity')}</b>${p.rolls.map(r=>esc(r.label)).join(' → ')}<br><span>${p.phase==='aging'?`${p.months} months have passed. Final aging check.`:'Follow-up rolls resolve this event; they add no extra time.'}</span>`;}
 $('oddsModifier').hidden=!s?.modifier;
 if(s?.modifier){const m=s.modifier;$('oddsModifier').innerHTML=`<div class="modifier-title">FATE ROLLED · ${esc(m.label)}</div><strong>${esc(m.outcome)}: ${pct(m.before)} → <span class="delta">${pct(m.after)}</span></strong><p>+${(m.after-m.before).toFixed(1)} percentage points. Other outcomes are rebalanced proportionally. No player choice was made.</p>`;}
}
function summary(){
