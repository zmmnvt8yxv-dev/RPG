...
import {worldMapSummary} from './world-map.js';
...
function heritageRows(a){
 const h=heritageProfile(a),names={fightingMastery:'Fighting Mastery',battleIQ:'Battle IQ',strength:'Strength',durability:'Durability',speed:'Speed',endurance:'Endurance',stamina:'Stamina'};
 const bonuses=Object.entries(h.bonuses).map(([k,v])=>`+${v} ${names[k]||k}`).join(' · ');
 const haki=h.guaranteedHaki.map(k=>k==='haki_observation'?'Observation Haki':k==='haki_armament'?'Armament Haki':"Conqueror’s Haki").join(' + ');
 return {bonuses,styles:h.styles.join(' + '),haki};
}
function sheet(){
 ...
 const heritageView=heritageRows(a);
 $('characterSheet').innerHTML=originGroups.map(g=>{const rows=records.filter(h=>h.group===g);return rows.length?`<section class="sheet-group"><h3>${g.toUpperCase()}</h3><dl>${rows.map(h=>row(labels[h.id]||h.label,h.value,h.note)).join('')}</dl></section>`:'';}).join('')+(a.race?`<section class="sheet-group"><h3>HERITAGE</h3><dl>${heritageView.bonuses?row('Attribute shifts',heritageView.bonuses,'Applied as full starting-rank increases when the journey begins.'):row('Attribute shifts','None')}${heritageView.haki?row('Guaranteed Haki',heritageView.haki):''}${heritageView.styles?row('Secondary style',heritageView.styles):''}</dl></section>`:'');
...
function renderStory(){
 const j=journey;
 if(!j.startLocationResolved){
  $('storyCompass').innerHTML='<div class="compass-heading"><span class="eyebrow">JOURNEY SETUP / WORLD MAP</span><span class="life-stage">Chapter 0</span></div><h2>First, fate chooses your place in the world.</h2><p>Your path, family, crew affiliation and age shape the odds. This spin advances no time and creates no chapter.</p>';
  return;
 }
 const life=lifeContext(j),d=dreamOf(j),r=dreamReadiness(j);
